...
import { ShoppingBag, Trash2, Minus, Plus, Coffee, Cake } from "lucide-react";
      ...
      <div className="mb-6">
        {cart.map(item => <div key={item.id} className="flex border-b border-gray-200 py-4">
            <div className="w-20 h-20 bg-gray-100 rounded-md mr-4 overflow-hidden">
              {!imageErrors[item.id] ? <img src={item.image} alt={item.name} className="w-full h-full object-cover" onError={() => setImageErrors(prev => ({
            ...prev,
            [item.id]: true
          }))} loading="lazy" /> : <div className="w-full h-full flex items-center justify-center">
                  {item.type === "coffee" ? <Coffee className="text-gray-400" size={32} /> : <Cake className="text-gray-400" size={32} />}
                </div>}
            </div>
